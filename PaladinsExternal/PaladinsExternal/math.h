...
#define Const_URotationToRadians  Const_PI / Const_URotation180 
	...
	bool W2S(FVector target, Vec2& dst, FRotator myRot, FVector myLoc, float fov)
	{
		...
		float GFOV = fov;
		float CentX = (GameVars.ScreenWidth / 2.0f);
		float CentY = (GameVars.ScreenHeight / 2.0f);
		...
		if (dst.x >= 0.0f && dst.x <= GameVars.ScreenWidth)
		{
			if (dst.y >= 0.0f && dst.y <= GameVars.ScreenHeight)
			{
				return true;
			}
		}
